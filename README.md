...
```bash
git clone https://github.com/yourusername/finance-bot
cd finance-bot
```
...
Бот умеет показывать премиум-эмодзи из твоего набора. Это работает, потому что
владелец бота (ты) имеет Telegram Premium — отправлять кастомные эмодзи может
бот, чей владелец премиум; зрителям Premium для просмотра не нужен.

**Как включить:**
1. Создай набор эмодзи через [@stickers](https://t.me/stickers) (нужен Premium).
2. Узнай `custom_emoji_id`: отправь своё кастомное эмодзи боту и посмотри
...
Отключить полностью: `PREMIUM_EMOJI=0` в `fin.env`.
